...
- **One thing does leave the device, and it isn't us: Android Auto Backup.** "No network calls" is true of the app's own code — it holds no Internet permission — but the OS backs up app data to the user's Google account by default, so the zone list *is* copied off the device by Android itself. That is deliberate (re-picking your zones is the only setup this app has), and it is scoped to an explicit allowlist rather than left at the default "back up the whole data directory": exactly the chosen zone list and the first-run flag, in [`backup_rules.xml`](android/app/src/main/res/xml/backup_rules.xml) and [`data_extraction_rules.xml`](android/app/src/main/res/xml/data_extraction_rules.xml). No message text and nothing from contacts is persisted, so nothing of that kind can be included. To opt out entirely, turn off backup for TimeTwister in Android Settings, or build with `android:allowBackup="false"`.
